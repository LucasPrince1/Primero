...
import java.util.*;
// ArryaList 하나를 준비해서 이곳에 데이터를 넣고 빼고를 할 예정이다.
...
public class Sync02 {
	...
	//리스트에 데이터를 쌓는 함수
	synchronized public void makeData(int num) {
		if(list.size() == 5) {
			//이 경우 데이터가 5개가 채워져 있으므로 작업을 하면 안된다.
			try {
		...
		System.out.println("<> 채운후 데이터의 개수는 " + list.size() + " 개가 있습니다.");
		//이렇게 숫자를 채우면 숫자를 사용할 수 있는 상태가 된다.
		notify();
		// ==> 가장 먼저 Block 상태로 들어간 스레드가 일을 하게 된다.
	...
	synchronized void useData() {
		...
		int no = list.get(0);
		//꺼낸 데이터는 리스트에서 제거한다.
		list.remove(0);	// 꺼낸 데이터를 제거했으므로 입력할 수 있는 공간이 하나가 더 생겨났다.
...
class DataThread01 extends Thread {
	Sync02 main; //주소 기억할 준비
	
	public DataThread01() {}
	public DataThread01(Sync02 main) {
		this.main = main;
	}
...
class DataThread02 extends Thread {
	...
	public DataThread02() {}
	public DataThread02(Sync02 main) {
		this.main = main;
	}
